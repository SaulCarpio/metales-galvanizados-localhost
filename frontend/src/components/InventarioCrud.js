...
const InventarioCrud = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await inventarioAPI.list(); // Axios devuelve res.data
      if (res.data.success && res.data.inventario) {
        setItems(res.data.inventario);
      } else {
        setItems([]);
        console.warn("No hay inventario disponible");
      }
    } catch (e) {
